...
class CurrentInputState:
    ...
    def filter_dataset(
        ...
        filters: dict[str | BusinessModelName, Callable[[BusinessModel], bool]] = None,
    ) -> AtlasDataset:
        ...
        for object_type in included_types:
            ...
            if not filters or object_type_str not in filters:
                try:
                    container: Container[BusinessModel] = self.data.get_container_by_type(object_type_str)
        ...
        if filters:
            for object_type, filter_fn in filters.items():
                ...
                try:
                    container: Container[BusinessModel] = self.data.get_container_by_type(object_type_str)
                    filtered_data[object_type_str] = [obj for obj in container if filter_fn(obj)]
                except ValueError:
                    continue
