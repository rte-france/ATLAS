...
class AbstractParameters(BaseModel):
    ...
    :param end_date: Study start date
    ...
    def check_dates(self):
        if self.end_date < self.start_date:
            raise ValueError(f"Start date '{self.start_date.strftime('%Y-%m-%d %H:%M:%S')}' must be inferior "
                             f"to end date '{self.end_date.strftime('%Y-%m-%d %H:%M:%S')}'")
        if not (self.start_date < self.execution_date < self.end_date):
            raise ValueError(f"Execution date '{self.execution_date.strftime('%Y-%m-%d %H:%M:%S')}' must be between "
                             f"start date '{self.start_date.strftime('%Y-%m-%d %H:%M:%S')}' and "
