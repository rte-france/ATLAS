...
class AbstractOrchestratorParameters(Parameters):
    ...
    :type orchestrator_path: Path | None
    ...
    def validate_use_of_relative_path(self) -> "AbstractOrchestratorParameters":
        ...
        if self.__class__.__name__ == Workflow.__class__.__name__:
            class_name = Workflow.__class__.__name__
            alias_path_from_orchestrator = "path_from_workflow"
            alias_orchestrator_path = "workflow_path"
        elif self.__class__.__name__ == ActionPlan.__class__.__name__:
            class_name = ActionPlan.__class__.__name__
            alias_path_from_orchestrator = "path_from_action_plan"
            alias_orchestrator_path = "action_plan_path"

        if self.orchestrator_path is None:
            raise DataValidationError(
                f"{class_name} {self.name} has parameter {alias_path_from_orchestrator} (or \"path_from_orchestrator\") "
                f"set to True while parameter {alias_orchestrator_path} (or \"orchestrator_path\") is not defined or None."
            )
        elif not self.orchestrator_path.absolute():
            raise DataValidationError(
                f"Path {self.orchestrator_path} is not an absolute path."
                f"This path used for {class_name} {self.name} parameter {alias_orchestrator_path} (or \"orchestrator_path\") and has to be an absolute path."

            )
        elif not self.orchestrator_path.exists():
            raise DataValidationError(
                f"Path {self.orchestrator_path} doesn't exist."
                f"This path used for {class_name} {self.name} parameter {alias_orchestrator_path} (or \"orchestrator_path\") and has to be an existing path."
            )
        return self
