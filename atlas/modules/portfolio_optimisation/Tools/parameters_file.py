...
from PO_functions import get_parameter_date, get_deltaT_to_string, get_date_to_clean_string, check_output_path
import API
import os
...
class Parameters(object):
    ...
    __slots__ = (
        ...
        "time_out",
        "duality_gap",
        "allowed_round_off_error",
        "debug",
        "verbose",
        "optimization_period",
        "thermal_optimization_period",
        "hydraulic_optimization_period",
        "battery_optimization_period",
        "phs_optimization_period",
        "ev_optimization_period",
        "op_times",
        "thermal_op_times",
        "hydraulic_op_times",
        "battery_op_times",
        ...
        "ev_op_times",
        "init_battery_time",
        "small_imbalance_size",
        "small_imbalance_penalty",
        "large_imbalance_penalty",
        "max_overall_imbalance",
        "imbalance_penalty_offset",
        "ev_smoothing_factor",
        "ev_nb_fragments",
        "ev_additional_hours",
        "ev_initial_level",
        "ev_reserve_duration",
        "battery_smoothing_factor",
        "battery_nb_fragments",
        "battery_additional_hours",
        "automated_battery_reserve_duration",
        "battery_initial_level",
        "battery_reserve_duration",
        "phs_smoothing_factor",
        "phs_nb_fragments",
        "phs_additional_hours",
        "phs_initial_level",
        "phs_reserve_duration",
        "hydro_minimal_fragment_size",
        "priceForecastsTypes",
        "automated_unprocured_reserves_penalty",
    ...
    def __init__(self, output_marker):
        ...
        self.execution_date = DateTime(
            execution_date.Year, execution_date.Month, execution_date.Day, execution_date.Hour, execution_date.Minute, 0
        )
        ...
        if self.market not in ["DayAhead", "Intraday", "RRActivation", "MFRRActivation"]:
            API.IO.Trace.Log(
                "Market parameter is invalid. Please look at its description for more informations", API.IO.LogTypeError
            )
    ...
    def __str__(self):
        return (
            ...
            " - TimeOut: {}sec\n"
            " - DualityGap: {}\n"
            " - AllowedRoundOffError: {}\n"
            " - ISPForecastLowerBound: {}euro/MWh\n"
            " - SmallImbalancePenalty: {}euro/MWh\n"
            " - LargeImbalancePenalty: {}euro/MWh\n"
            " - AutomatedUnprocuredReservesPenalty: {} euros/MWh \n"
            " - ManualUnprocuredReservesPenalty: {} euros/MWh \n"
            " - ElectricVehicleSmoothingFactor: {}\n"
            " - ElectricVehicleNumberOfFragments: {}\n"
            " - BatterySmoothingFactor: {}\n"
            " - BatteryNumberOffFragments: {}\n"
            " - PumpedHydraulicStorageSmoothingFactor: {}\n"
            " - PumpedHydraulicStorageNumberOfFragments: {}\n"
            " - HydraulicMinimalFragmentSize: {} MW \n"
            " - WithRounding: {}".format(
                self.start_date,
                self.end_date,
                self.execution_date,
                ...
                self.market,
                self.use_forecast,
                self.is_portfolio_bidding,
                self.solver,
                self.presolve,
                self.time_out,
                self.duality_gap,
                self.allowed_round_off_error,
                self.isp_forecast_lower_bound,
                self.small_imbalance_penalty,
                self.large_imbalance_penalty,
                self.automated_unprocured_reserves_penalty,
                self.manual_unprocured_reserves_penalty,
                self.ev_smoothing_factor,
                self.ev_nb_fragments,
                self.battery_smoothing_factor,
                self.battery_nb_fragments,
                self.phs_smoothing_factor,
                self.phs_nb_fragments,
                self.hydro_minimal_fragment_size,
                self.with_rounding,
            )
        )
