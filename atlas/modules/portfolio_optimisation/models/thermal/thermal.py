...
class ThermalPO(BaseEquipmentPO, Thermal):
    ...
    def _compute_time_parameters(self, parameters: PortfolioOptimisationParameters):
        ...
        if self.minimum_time_on and self.minimum_time_on.total_minutes() > 0:
            self._T_on = int(max(1, math.ceil(self.minimum_time_on / parameters.date.timestep))) + 1
        else:
            self._T_on = 0

        if self.minimum_time_off and self.minimum_time_off.total_minutes() > 0:
            self._T_off = int(max(1, math.ceil(self.minimum_time_off / parameters.date.timestep))) + 1
        else:
            self._T_off = 0

        if self.startup_duration:
            self._T_start = int(math.floor(self.startup_duration / parameters.date.timestep))
        else:
            self._T_start = 0

        if self.shutdown_duration:
            self._T_stop = int(math.floor(self.shutdown_duration / parameters.date.timestep))
        else:
            self._T_stop = 0

        if self.minimum_stable_power_duration:
            if self.minimum_stable_power_duration < parameters.date.timestep:
                self._T_stable = 0
            else:
                self._T_stable = int(math.ceil(self.minimum_stable_power_duration / parameters.date.timestep)) + 1
                self._T_stable = self._T_stable if self._T_stable >= 2 else 0
        else:
            self._T_stable = 0

        self._Delta_Q = self.maximum_gradient * parameters.date.timestep.total_minutes()
        ...
        if time in self.optimisation_time_window:
            ...
            if self.minimum_power is None:
                self.minimum_power = Timeseries.from_index(
                    ...
                    frequency=parameters.date.timestep,
                    default_value=0,
                )
    ...
    def add_objective(
        ...
        if time in self.optimisation_time_window:
            ...
            model.add_objective(
                variable_cost * power_level_var * parameters.date.timestep.total_hours(),
            )

            if time > max(parameters.target_times):
                model.add_objective(
                    -price_forecast * power_level_var * parameters.date.timestep.total_hours(),
                )
    ...
    def _get_initial_time_window(
        ...
        if self.T_traceback > 0:
            for k in range(self.T_traceback, 0, -1):
                initial_times.append(parameters.date.start_date - k * parameters.date.timestep)
        else:
            initial_times.append(parameters.date.start_date - parameters.date.timestep)

        for k in range(self.T_traceback, 1, -1):
            stable_initial_times.append(parameters.date.start_date - k * parameters.date.timestep)
    ...
    def _add_initial_variables(self, parameters: PortfolioOptimisationParameters):
        ...
        if self._T_stable >= 1:
            self.on_up_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
            self.on_down_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
            self.on_flat_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
            self.stable_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
            self.entered_up_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
            self.entered_down_var.set_model_var(parameters.date.start_date - parameters.date.timestep)

        if self._T_stable >= 1 and (self._T_start >= 1 or self._T_stop >= 1):
            self.dd_grad_var.set_model_var(parameters.date.start_date - parameters.date.timestep)
    ...
    def add_initial_conditions(
        ...
        if power_ts is not None:
            if parameters.date.start_date - parameters.date.timestep != power_ts.last_date():
                day_zero = True
