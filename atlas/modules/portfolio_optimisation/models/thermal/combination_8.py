...
def add_constraints(
    ...
    on_up_2_prev_var = obj.on_up_var.get_value(prev_time - parameters.temporal.timestep)
    ...
    if time == parameters.temporal.start_date:
        model.add_constraint(
            stable_prev_var <= on_flat_2_prev_var,
        ...
        model.add_constraint(
            ...
            f"transition_constraint_3_{prev_time}_{obj.name}",
        )
        model.add_constraint(
            stop_2_prev_var + on_down_prev_var <= 1,
            f"transition_constraint_4_{prev_time}_{obj.name}",
        )
        model.add_constraint(
    ...
    stop_eviction_time = time - (obj._T_stop - 1) * parameters.temporal.timestep
    turned_off_stop_eviction_var = obj.turned_off.get_value(stop_eviction_time)
    model.add_constraint(
        turned_off_stop_eviction_var + stop_var <= 1,
        f"stop_eviction_constraint_{time}_{obj.name}",
    )

    start_eviction_time = time - (obj._T_start - 1) * parameters.temporal.timestep
    turned_on_start_eviction_var = obj.turned_on.get_value(start_eviction_time)
    model.add_constraint(
        turned_on_start_eviction_var + start_var <= 1,
        f"start_eviction_constraint_{time}_{obj.name}",
    )
    ...
    if obj._T_on >= 2:
        for s in range(1, obj._T_on):
            ...
            if time == parameters.temporal.start_date:
                local_time = time - (s + obj._T_start + 1) * parameters.temporal.timestep
                turned_on_local_var = obj.turned_on.get_value(local_time)
                model.add_constraint(
    ...
    model.add_constraint(
        reserves_up_var <= max_power * (1 - on_up_var - on_down_var - off_var - start_var - stop_var),
