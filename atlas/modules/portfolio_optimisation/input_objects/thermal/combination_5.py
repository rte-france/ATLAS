    ...
    **kwargs,
    ...
    if day_zero:
        ...
        for idx, time in enumerate(kwargs.get("stable_initial_times", [])):
            ...
            if idx >= 2:
                initialize_flat_down_stop_initial_conditions(
                    ...
                    time - 2 * parameters.temporal.timestep,
                )
        ...
        initialize_flat_down_stop_initial_conditions(
            ...
            parameters.temporal.start_date - 2 * parameters.temporal.timestep,
            parameters.temporal.start_date - 3 * parameters.temporal.timestep,
        )
...
def add_constraints(
    ...
    dd_grad_prev_var = obj.dd_grad_var.get_value(prev_time)

    off_prev_var = obj.off_var.get_value(prev_time)
    on_up_prev_var = obj.on_up_var.get_value(prev_time)
    on_up_2_prev_var = obj.off_var.get_value(prev_time - parameters.temporal.timestep)
    on_down_prev_var = obj.on_down_var.get_value(prev_time)
    on_down_2_prev_var = obj.on_down_var.get_value(prev_time - parameters.temporal.timestep)
    on_flat_prev_var = obj.on_flat_var.get_value(prev_time)
    stop_prev_var = obj.stop_var.get_value(prev_time)
    stop_2_prev_var = obj.stop_var.get_value(prev_time - parameters.temporal.timestep)
    ...
    on_flat_2_prev_var = obj.on_flat_var.get_value(prev_time - parameters.temporal.timestep)
    ...
    model.add_constraint(flat_down_stop_var <= stop_var, f"flat_down_stop_evol_1_{time}_{obj.name}")
    model.add_constraint(
        flat_down_stop_var <= on_down_prev_var,
        f"flat_down_stop_evol_2_{time}_{obj.name}",
    )
    model.add_constraint(
        flat_down_stop_var <= on_flat_2_prev_var,
        f"flat_down_stop_evol_3_{time}_{obj.name}",
    )
    model.add_constraint(
        flat_down_stop_var >= stop_var + on_down_prev_var + on_flat_2_prev_var - 2,
        f"flat_down_stop_evol_4_{time}_{obj.name}",
    )
    ...
    model.add_constraint(
        off_var + on_up_var + on_down_var + on_flat_var + stop_var == 1,
    ...
    model.add_constraint(on_up_prev_var + off_var <= 1, f"transition_constraint_3_{time}_{obj.name}")
    model.add_constraint(on_down_prev_var + off_var <= 1, f"transition_constraint_4_{time}_{obj.name}")
    model.add_constraint(stop_prev_var + on_flat_var <= 1, f"transition_constraint_5_{time}_{obj.name}")
    model.add_constraint(stop_prev_var + on_down_var <= 1, f"transition_constraint_6_{time}_{obj.name}")
    model.add_constraint(stop_prev_var + on_up_var <= 1, f"transition_constraint_7_{time}_{obj.name}")
    ...
    eviction_time = time - (obj._T_stop - 1) * parameters.temporal.timestep
    ...
    if obj._T_on >= 2:
        for s in range(1, obj._T_on):
            ...
            if time == parameters.temporal.start_date:
                local_time = time - (s + 1) * parameters.temporal.timestep
                turned_on_local_var = obj.turned_on.get_value(local_time)
                model.add_constraint(
    ...
    if obj._T_off >= 2:
        for s in range(1, obj._T_off):
            local_time = time - (s + obj._T_stop) * parameters.temporal.timestep
    ...
    model.add_constraint(
        reserves_up_var <= max_power * (1 - on_up_var - on_down_var - off_var - stop_var),
        f"reserves_up_max_{time}_{obj.name}",
    )
    model.add_constraint(
        reserves_down_var <= max_power * (1 - on_up_var - on_down_var - off_var - stop_var),
    ...
    if time == parameters.temporal.start_date:
        ...
        model.add_constraint(
            off_prev_var + on_up_prev_var + on_down_prev_var + on_flat_prev_var + stop_prev_var == 1,
        ...
        model.add_constraint(
            stop_2_prev_var + on_flat_prev_var <= 1,
            f"transition_constraint_5_{prev_time}_{obj.name}",
        )
        model.add_constraint(
            stop_2_prev_var + on_down_prev_var <= 1,
            f"transition_constraint_6_{prev_time}_{obj.name}",
        )
        model.add_constraint(
            stop_2_prev_var + on_up_prev_var <= 1,
    ...
    if time in obj.optimisation_time_window[:-2]:
        ...
        model.add_constraint(
            dd_grad_prev_var <= down_grad_prev_var - min_power * (1 - stop_var),
            f"DD_evol_3_{time}_{obj.name}",
        )
        model.add_constraint(
            dd_grad_prev_var >= down_grad_prev_var - max_power * (1 - stop_var),
            f"DD_evol_4_{time}_{obj.name}",
        )
