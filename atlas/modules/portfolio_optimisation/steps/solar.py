...
class SolarStep(AbstractOptimStep[SolarPO, "PortfolioOptimisationParameters"]):
    def add_variables(self, model: OptimisationModel, parameters: PortfolioOptimisationParameters):
        eq = self.equipment
        for time in eq.optimisation_time_window:
            cfg.logger.debug(f"Adding variables for solar unit {eq.name} at time {time}")
    ...
    def add_constraints(self, model: OptimisationModel, parameters: PortfolioOptimisationParameters):
        eq = self.equipment
        for time in eq.optimisation_time_window:
            cfg.logger.debug(f"Adding constraints for solar unit {eq.name} at time {time}")
            max_power = eq._cached_forecast.get_value(time) if eq._cached_forecast else 0
            min_power = (1 - eq.maximum_curtailment_ratio.get_value(time)) * max_power
            maximum_automated = get_maximum_automated(eq)
    ...
    def add_objective(
        ...
        eq = self.equipment
        for time in eq.optimisation_time_window:
            cfg.logger.debug(f"Adding objective for solar unit {eq.name} at time {time}")
            power_level_var = model.get_variable(f"{eq.name}_power_level_{time}")
            model.add_objective(
