...
class IntradayPriceForecastInputDataset(AbstractDataset[IntradayPriceForecastParameters]):
    def __init__(self, input_data: AtlasDataset, parameters: IntradayPriceForecastParameters):
        ...
        self.solar: list[SolarIDPF] = [SolarIDPF.model_validate(obj, context=context) for obj in input_data.solar]
        self.wind: list[WindIDPF] = [WindIDPF.model_validate(obj, context=context) for obj in input_data.wind]
        self.load: list[LoadIDPF] = [LoadIDPF.model_validate(obj, context=context) for obj in input_data.load]
