...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    for t in model.time_frame:
        ...
        # Enforces eq. (9)
        model.add_constraint(
            model.OFF.get_value(t) + model.ON_UP[t] + model.ON_DOWN.get_value(t) + model.START[t] == 1,
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(model.ON_DOWN.get_value(t_minus_one) + model.START[t] <= 1)  # eq. (10)
        model.add_constraint(model.START[t_minus_one] + model.OFF.get_value(t) <= 1)  # eq. (11)
        model.add_constraint(model.OFF.get_value(t_minus_one) + model.ON_UP[t] <= 1)  # eq. (15)
        model.add_constraint(
    ...
    if model.T_on >= 2:
        for t in model.time_frame:
            ...
            for s in time_steps:
                ...
                model.add_constraint(
                    model.turned_on[t_minus_s_minus_T_start] <= model.ON_UP[t] + model.ON_DOWN.get_value(t),
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(
            ...
            <= model.q_upper.get_value(t) * (1 - model.OFF.get_value(t) - model.START[t])
        )
        model.add_constraint(
    ...
    for t in model.time_frame:
        model.add_constraint(
            model.q[t] >= model.q_lower.get_value(t) * (model.ON_UP[t] + model.ON_DOWN.get_value(t)),
            f"lower_bound_of_{model.thermal_unit.name}_at_{t}",
        )  # Lower bound (eq. (33))
        model.add_constraint(
            model.q[t]
            <= model.q_upper.get_value(t) * (model.ON_UP[t] + model.ON_DOWN.get_value(t)) + model.START[t] * q_min,
