...
def execute(model: ThermalOptimization) -> None:
    ...
    if day_zero:
        ...
        for t in model.previous_time_frame:
            if model.last_power.get_value(t) >= model.thermal_unit.minimum_power.get_value(t):
                model.OFF[t] = 0  # Only the OFF and STOP variables are initialized. ON_FLAT, ON_DOWN and ON_UP will be
        ...
        for t in model.previous_time_frame[:-2]:
            ...
            flat_down_stop[t] = int(
                math.floor((model.STOP[t] + model.ON_DOWN[t_minus_one] + model.ON_FLAT[t_minus_two]) / 3)
            )
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(flat_down_stop[t] <= model.ON_DOWN[t_minus_one])
        model.add_constraint(flat_down_stop[t] <= model.ON_FLAT[t_minus_two])
        model.add_constraint(
            flat_down_stop[t] >= model.STOP[t] + model.ON_DOWN[t_minus_one] + model.ON_FLAT[t_minus_two] - 2
        )
    ...
    for t in model.time_frame_union_minus_one:
        ...
        # Enforces eq. (9)
        model.add_constraint(
            model.OFF[t] + model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t] + model.STOP[t] == 1,
    ...
    for t in model.time_frame_union_minus_one:
        ...
        model.add_constraint(model.ON_DOWN[t_minus_one] + model.ON_UP[t] <= 1)
        # Eq (13)
        ...
        model.add_constraint(
            model.STOP[t_minus_one] + model.ON_UP[t] <= 1,
            f"transitions_constraints_on_timeFrame_union_minus_one_at_{t}",
        )
    for t in model.time_frame:
        t_minus_one = t - model.parameters.time_step
        # ON_UP to STOP transition (eq. (21))
        model.add_constraint(model.ON_UP[t_minus_one] + model.STOP[t] <= 1)
        # Eq. (12)
        model.add_constraint(
            model.OFF[t_minus_one] + model.STOP[t] <= 1,
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(
            ...
            <= model.q_upper.get_value(t) * (1 - model.ON_UP[t] - model.ON_DOWN[t] - model.OFF[t] - model.STOP[t])
        )
    ...
    for t in model.time_frame:
        model.add_constraint(
            model.q[t]
            >= model.q_lower.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t])
        ...
        model.add_constraint(
            model.q[t]
            <= model.q_upper.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t])
            + model.STOP[t] * q_min
        ...
        for t in model.gradients_time_frame:  # The gradients are defined only up to T-1.
            ...
            model.add_constraint(
                ...
                <= (
                    ...
                    + model.U[t]
                    + model.D[t]
                    - q_step * model.turned_off[t_next]
                    - model.STOP[t] * q_step
