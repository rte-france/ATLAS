...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    if day_zero:
        ...
        for t in model.previous_time_frame:
            if model.last_power.get_value(t) >= model.thermal_unit.minimum_power.get_value(t):
                model.OFF.set_extended(
                    t, 0
                )  # Only the OFF and STOP variables are initialized. ON_FLAT, ON_DOWN and ON_UP will be
    ...
    for t in model.time_frame:
        ...
        # Eq. (12)
        model.add_constraint(
            model.OFF.get_value(t_minus_one) + model.STOP.get_value(t) <= 1,
    ...
    if model.T_on >= 2:
        for t in model.time_frame_union_minus_one:
            ...
            for s in time_steps:
                ...
                model.add_constraint(
                    model.turned_on.get_value(t_minus_s)
    ...
    if model.T_stable >= 2:
        for t in model.time_frame_union_minus_one:
            time_steps = range(1, model.T_stable - 1)  # Corresponds to the set {1,..., model.T_stable - 1}
            for s in time_steps:
                # Enforces eq. (26)
                t_minus_s = t - s * model.parameters.temporal.timestep
                model.add_constraint(
    ...
    for t in model.time_frame:
        model.add_constraint(
            ...
            <= model.maximum_automated * (1 - model.OFF.get_value(t) - model.STOP.get_value(t))
        )
        model.add_constraint(
    ...
    for t in model.time_frame:
        model.add_constraint(
            ...
            >= model.q_lower.get_value(t)
            * (model.ON_UP.get_value(t) + model.ON_DOWN.get_value(t) + model.ON_FLAT.get_value(t))
            + model.turned_off.get_value(t) * (q_min - q_step),
        ...
        model.add_constraint(
            ...
            <= model.q_upper.get_value(t)
            * (model.ON_UP.get_value(t) + model.ON_DOWN.get_value(t) + model.ON_FLAT.get_value(t))
            + model.STOP.get_value(t) * q_min
            - model.turned_off.get_value(t) * q_step,
