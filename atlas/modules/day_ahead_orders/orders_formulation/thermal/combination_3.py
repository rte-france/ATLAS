...
def execute(model: ThermalOptimization) -> None:
    """Combination 3 : T_stop = 0, model.T_stable >= 1 T_start = 0"""
    ...
    if model.is_day_zero():
        ...
        for t in model.previous_time_frame:
            if model.last_power.get_value(t) > 0:
                model.OFF[t] = 0  # Only the OFF variable is initialized. ON_FLAT, ON_DOWN and ON_UP will be
        ...
        for t in model.previous_time_frame:
            ...
            if not t == model.extended_start_date:
                ...
                if model.OFF[t] - model.OFF[t_prev] == 1:
        ...
        for t in model.previous_time_frame[
            ...
            if (not t == model.extended_start_date) and (not model.OFF[t] == 1):
                t_prev = t - model.parameters.time_step
                ...
                if model.ON_FLAT[t] - model.ON_FLAT[t_prev] == 1:
                    model.stable[t] = 1
                # or the UP state
                if model.ON_UP[t] - model.ON_UP[t_prev] == 1:
                    model.entered_up[t] = 1
                # or the DOWN state
                if model.ON_DOWN[t] - model.ON_DOWN[t_prev] == 1:
    ...
    model.U[model.start_date_minus_one] = (
        model.ON_UP[model.start_date_minus_one]
        * model.ON_UP[start_date_minus_two]
        * (model.q[model.start_date_minus_one] - model.q[start_date_minus_two])
    )
    model.D[model.start_date_minus_one] = (
        model.ON_DOWN[model.start_date_minus_one]
        * model.ON_DOWN[start_date_minus_two]
        * (model.q[model.start_date_minus_one] - model.q[start_date_minus_two])
    )
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(model.turned_on[t] >= model.OFF[t - model.parameters.time_step] - model.OFF[t])

        # Constraints on turned_off
    # Enforces eq. (4) as there is no STOP state in this case.
    for t in model.time_frame:
        model.add_constraint(model.turned_off[t] <= 1 - model.OFF[t - model.parameters.time_step])
        model.add_constraint(model.turned_off[t] <= model.OFF[t])
        model.add_constraint(model.turned_off[t] >= model.OFF[t] - model.OFF[t - model.parameters.time_step])
    ...
    for t in model.time_frame_union_minus_one:
        # entered_up (eq. (7))
        model.add_constraint(model.entered_up[t] <= 1 - model.ON_UP[t - model.parameters.time_step])
        model.add_constraint(model.entered_up[t] <= model.ON_UP[t])
        model.add_constraint(model.entered_up[t] >= model.ON_UP[t] - model.ON_UP[t - model.parameters.time_step])
        # entered_down (eq. (8))
        model.add_constraint(model.entered_down[t] <= 1 - model.ON_DOWN[t - model.parameters.time_step])
        model.add_constraint(model.entered_down[t] <= model.ON_DOWN[t])
        model.add_constraint(model.entered_down[t] >= model.ON_DOWN[t] - model.ON_DOWN[t - model.parameters.time_step])
    ...
    for t in model.time_frame:  # Loop in all the time_frame but start_date.
        ...
        # tilde_D (eq. (30))
        model.add_constraint(model.tilde_D[t] <= model.Q_max * model.ON_DOWN[t_minus_one])
        model.add_constraint(model.tilde_D[t] >= model.Q_min * model.ON_DOWN[t_minus_one])
        model.add_constraint(
            model.tilde_D[t] <= model.q[t] - model.q[t_minus_one] - model.Q_min * (1 - model.ON_DOWN[t_minus_one])
        )
        model.add_constraint(
            model.tilde_D[t] >= model.q[t] - model.q[t_minus_one] - model.Q_max * (1 - model.ON_DOWN[t_minus_one]),
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(model.D[t] <= model.tilde_D[t] - model.Q_min * (1 - model.ON_DOWN[t]))
        model.add_constraint(
            model.D[t] >= model.tilde_D[t] - model.Q_max * (1 - model.ON_DOWN[t]),
    ...
    for t in model.time_frame_union_minus_one:
        ...
        # Enforces eq. (9)
        model.add_constraint(
            model.OFF[t] + model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t] == 1,
    ...
    for t in model.time_frame_union_minus_one:
        t_minus_one = t - model.parameters.time_step
        # Implement eq. (25).
        model.add_constraint(model.ON_UP[t_minus_one] + model.ON_DOWN[t] <= 1)
        model.add_constraint(
            model.ON_DOWN[t_minus_one] + model.ON_UP[t] <= 1,
    ...
    if model.T_on >= 2:
        for t in model.time_frame_union_minus_one:
            ...
            for s in time_steps:
                ...
                model.add_constraint(
                    model.turned_on[t_minus_s] <= model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t],
    ...
    if model.T_off >= 2:
        for t in model.time_frame:
            ...
            for s in time_steps:
                ...
                t_minus_s = t - s * model.parameters.time_step
                model.add_constraint(
                    model.turned_off[t_minus_s] <= model.OFF[t],
    ...
    for t in model.time_frame:
        model.add_constraint(
            ...
            <= model.q_lower.get_value(t) * (1 - model.ON_UP[t] - model.ON_FLAT[t] - model.ON_DOWN[t])
        )

    # impossible commitment and stable reserves constraints (eqs. (44) and (45))
    for t in model.time_frame:
        model.add_constraint(
            model.get_variable(model.automated_reserves_up_at(t)) <= model.maximum_automated * (1 - model.OFF[t])
        )
        model.add_constraint(
            model.get_variable(model.automated_reserves_down_at(t)) <= model.maximum_automated * (1 - model.OFF[t])
        )
        model.add_constraint(
            model.get_variable(model.reserves_up_equip_at(t))
            <= model.q_upper.get_value(t) * (1 - model.ON_UP[t] - model.ON_DOWN[t] - model.OFF[t])
    ...
    for t in model.time_frame:
        model.add_constraint(
            model.q[t] >= model.q_lower.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t]),
            f"lower_bound_of_{model.thermal_unit.name}_at_{t}",
        )  # Lower bound (eq. (33))

        model.add_constraint(
            model.q[t] <= model.q_upper.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t] + model.ON_FLAT[t]),
