...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    if day_zero:
        ...
        for t in model.previous_time_frame:
            if model.last_power.get_value(t) > 0:
                model.OFF.set_extended(t, 0)  # Only the OFF variable is initialized. ON_FLAT, ON_DOWN and ON_UP will be
    ...
    for t in model.time_frame:
        model.add_constraint(
            model.turned_off.get_value(t) <= 1 - model.OFF.get_value(t - model.parameters.date.timestep)
        )
        model.add_constraint(model.turned_off.get_value(t) <= model.OFF.get_value(t))
        model.add_constraint(
            model.turned_off.get_value(t)
            >= model.OFF.get_value(t) - model.OFF.get_value(t - model.parameters.date.timestep)
        )
    ...
    for t in model.time_frame_union_minus_one:
        ...
        # Enforces eq. (9)
        model.add_constraint(
            model.OFF.get_value(t) + model.ON_UP.get_value(t) + model.ON_DOWN.get_value(t) + model.ON_FLAT.get_value(t)
            == 1,
    ...
    for t in model.time_frame_union_minus_one:
        ...
        model.add_constraint(
            model.ON_DOWN.get_value(t_minus_one) + model.ON_UP.get_value(t) <= 1,
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(
            ...
            * (1 - model.ON_UP.get_value(t) - model.ON_DOWN.get_value(t) - model.OFF.get_value(t))
