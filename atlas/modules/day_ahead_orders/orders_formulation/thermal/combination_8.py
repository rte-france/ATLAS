...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    start_date_minus_two = model.parameters.temporal.start_date - 2 * model.parameters.temporal.timestep
    ...
    if day_zero:
        ...
        for t in model.previous_time_frame:
            ...
            if not t == model.start_date_minus_one:
                ...
                model.entered_up.set_extended(t, 0)
                model.entered_down.set_extended(t, 0)
        ...
        for t in model.previous_time_frame:
            if model.last_power.get_value(t) >= model.thermal_unit.minimum_power.get_value(t):
                ...
                model.OFF.set_extended(t, 0)
                model.STOP.set_extended(t, 1)  # Set both START and STOP at 1 for now, will be distinguished afterwards.
        ...
        for t in model.previous_time_frame[:-1]:
            ...
            if model.START.get_extended_value(t) == 1:  # Take start or stop, does not matter.
                if model.q.get_extended_value(t) > model.q.get_extended_value(
                    ...
                    model.STOP.set_extended(t, 0)
                    model.START.set_extended(t, 1)
                elif model.q.get_extended_value(t) < model.q.get_extended_value(
                    t_prev
                ):  # otherwise we are shutting down the unit.
                    model.STOP.set_extended(t, 1)
                    model.START.set_extended(t, 0)
        ...
        for t in model.previous_time_frame[
            ...
            if model.OFF.get_extended_value(t_prev) == 0:
                ...
                if model.q.get_extended_value(t) > model.q.get_extended_value(
                    t_prev
                ):  # Recall that here t_prev is earlier than t.
                    model.ON_UP.set_extended(t_prev, 1)
                    model.ON_DOWN.set_extended(t_prev, 0)
                    model.ON_FLAT.set_extended(t_prev, 0)
                elif model.q.get_extended_value(t) < model.q.get_extended_value(t_prev):
        ...
        for t in model.previous_time_frame[
            ...
            if (not t == model.extended_start_date) and (not model.OFF.get_extended_value(t) == 1):
                ...
                if model.ON_UP.get_extended_value(t) - model.ON_UP.get_extended_value(t_prev) == 1:
                    model.entered_up.set_extended(t, 1)
                # or the DOWN state
        ...
        for t in model.previous_time_frame[:-2]:
            ...
            t_minus_two = t - 2 * model.parameters.temporal.timestep
            flat_down_stop[t] = int(
                math.floor(
                    (
                        model.STOP.get_extended_value(t)
                        + model.ON_DOWN.get_extended_value(t_minus_one)
                        + model.ON_FLAT.get_extended_value(t_minus_two)
                    )
                    / 3
                )
            )
    ...
    for t in model.time_frame:
        model.add_constraint(
            model.turned_off.get_value(t) <= 1 - model.STOP.get_value(t - model.parameters.temporal.timestep)
        )
        model.add_constraint(model.turned_off.get_value(t) <= model.STOP.get_value(t))
        model.add_constraint(
            model.turned_off.get_value(t)
            >= model.STOP.get_value(t) - model.STOP.get_value(t - model.parameters.temporal.timestep)
        )
    ...
    for t in model.time_frame_union_minus_one:
        model.add_constraint(
            model.stable.get_value(t) <= 1 - model.ON_FLAT.get_value(t - model.parameters.temporal.timestep)
        )
        model.add_constraint(model.stable.get_value(t) <= model.ON_FLAT.get_value(t))
        model.add_constraint(
            model.stable.get_value(t)
            >= model.ON_FLAT.get_value(t) - model.ON_FLAT.get_value(t - model.parameters.temporal.timestep)
        )
    ...
    for t in model.time_frame:
        t_minus_one = t - model.parameters.temporal.timestep
        t_minus_two = t - 2 * model.parameters.temporal.timestep
        model.add_constraint(flat_down_stop[t] <= model.STOP.get_value(t))
        model.add_constraint(flat_down_stop[t] <= model.ON_DOWN.get_value(t_minus_one))
        model.add_constraint(flat_down_stop[t] <= model.ON_FLAT.get_value(t_minus_two))
        model.add_constraint(
            flat_down_stop[t]
            >= model.STOP.get_value(t) + model.ON_DOWN.get_value(t_minus_one) + model.ON_FLAT.get_value(t_minus_two) - 2
        )
    ...
    for t in model.time_frame_union_minus_one:
        # entered_up (eq. (7))
        model.add_constraint(
            model.entered_up.get_value(t) <= 1 - model.ON_UP.get_value(t - model.parameters.temporal.timestep)
        )
        model.add_constraint(model.entered_up.get_value(t) <= model.ON_UP.get_value(t))
        model.add_constraint(
            model.entered_up.get_value(t)
            >= model.ON_UP.get_value(t) - model.ON_UP.get_value(t - model.parameters.temporal.timestep)
        )
        # entered_down (eq. (8))
        model.add_constraint(
            model.entered_down.get_value(t) <= 1 - model.ON_DOWN.get_value(t - model.parameters.temporal.timestep)
        )
        model.add_constraint(model.entered_down.get_value(t) <= model.ON_DOWN.get_value(t))
        model.add_constraint(
            model.entered_down.get_value(t)
            >= model.ON_DOWN.get_value(t) - model.ON_DOWN.get_value(t - model.parameters.temporal.timestep)
        )
    ...
    for t in model.time_frame:  # Loop in all the time_frame but start_date.
        ...
        model.add_constraint(
            ...
            <= model.q.get_value(t)
            - model.q.get_value(t_minus_one)
            - model.Q_min * (1 - model.ON_UP.get_value(t_minus_one))
        )
        model.add_constraint(
            model.get_variable(model.aux_up_grad_at(t))
            >= model.q.get_value(t)
            - model.q.get_value(t_minus_one)
            - model.Q_max * (1 - model.ON_UP.get_value(t_minus_one)),
        ...
        model.add_constraint(
            ...
            <= model.q.get_value(t)
            - model.q.get_value(t_minus_one)
            - model.Q_min * (1 - model.ON_DOWN.get_value(t_minus_one))
        )
        model.add_constraint(
            model.get_variable(model.aux_down_grad_at(t))
            >= model.q.get_value(t)
            - model.q.get_value(t_minus_one)
            - model.Q_max * (1 - model.ON_DOWN.get_value(t_minus_one)),
    ...
    for t in model.gradients_time_frame:
        t_plus_one = t + model.parameters.temporal.timestep
    ...
    for t in model.time_frame_union_minus_one:
        ...
        # STOP to ON (eq. (13))
        model.add_constraint(model.STOP.get_value(t_minus_one) + model.ON_FLAT.get_value(t) <= 1)
        model.add_constraint(model.STOP.get_value(t_minus_one) + model.ON_DOWN.get_value(t) <= 1)
        model.add_constraint(
            model.STOP.get_value(t_minus_one) + model.ON_UP.get_value(t) <= 1,
            f"transitions_constraints_on_timeFrame_union_minus_one_at_{t}",
        )
    for t in model.time_frame:
        t_minus_one = t - model.parameters.temporal.timestep
        # ON_UP to STOP transition (eq. (21))
        model.add_constraint(model.ON_UP.get_value(t_minus_one) + model.STOP.get_value(t) <= 1)
        # OFF to STOP (eq. (13)).
        model.add_constraint(model.OFF.get_value(t_minus_one) + model.STOP.get_value(t) <= 1)
        # ON to START (eq. (10))
        ...
        model.add_constraint(model.START.get_value(t_minus_one) + model.STOP.get_value(t) <= 1)
        model.add_constraint(model.STOP.get_value(t_minus_one) + model.START.get_value(t) <= 1)
        # OFF to ON (eq. (15))
        model.add_constraint(model.OFF.get_value(t_minus_one) + model.ON_UP.get_value(t) <= 1)
        model.add_constraint(model.OFF.get_value(t_minus_one) + model.ON_FLAT.get_value(t) <= 1)
        model.add_constraint(
    ...
    for t in model.time_frame:
        t_minus_T_stop = t - model.T_stop * model.parameters.temporal.timestep
        t_minus_T_start = t - model.T_start * model.parameters.temporal.timestep
        # Implements equation (19)
        model.add_constraint(
            model.turned_off.get_value(t_minus_T_stop) + model.STOP.get_value(t) <= 1,
    ...
    for t in model.time_frame:
        model.add_constraint(
            ...
            <= model.maximum_automated
            * (1 - model.OFF.get_value(t) - model.START.get_value(t) - model.STOP.get_value(t))
        )
        model.add_constraint(
    ...
    for t in model.time_frame:
        model.add_constraint(
            ...
            >= model.q_lower.get_value(t)
            * (model.ON_UP.get_value(t) + model.ON_DOWN.get_value(t) + model.ON_FLAT.get_value(t))
            + model.turned_off.get_value(t) * (q_min - q_step_down),
        ...
        model.add_constraint(
            ...
            <= model.q_upper.get_value(t)
            * (model.ON_UP.get_value(t) + model.ON_DOWN.get_value(t) + model.ON_FLAT.get_value(t))
            + (model.STOP.get_value(t) + model.START.get_value(t)) * q_min
            - model.turned_off.get_value(t) * q_step_down,
        ...
        for t in model.gradients_time_frame:  # The gradients are defined only up to T-1.
            ...
            model.add_constraint(
                ...
                <= (
                    ...
                    + q_step_up * model.turned_on.get_value(t_next)
                    + model.START.get_value(t) * q_step_up
                    - DD[t]
                ),
