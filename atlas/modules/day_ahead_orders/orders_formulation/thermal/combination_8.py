...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    start_date_minus_two = model.parameters.date.start_date - 2 * model.parameters.date.timestep
    ...
    if day_zero:
        ...
        for t in model.previous_time_frame[:-2]:
            ...
            t_minus_two = t - 2 * model.parameters.date.timestep
    ...
    for t in model.time_frame_union_minus_one:
        model.add_constraint(
            model.stable.get_value(t) <= 1 - model.ON_FLAT.get_value(t - model.parameters.date.timestep)
        )
        model.add_constraint(model.stable.get_value(t) <= model.ON_FLAT.get_value(t))
        model.add_constraint(
            model.stable.get_value(t)
            >= model.ON_FLAT.get_value(t) - model.ON_FLAT.get_value(t - model.parameters.date.timestep)
        )
    ...
    for t in model.time_frame_union_minus_one:
        # entered_up (eq. (7))
        model.add_constraint(
            model.entered_up.get_value(t) <= 1 - model.ON_UP.get_value(t - model.parameters.date.timestep)
        )
        model.add_constraint(model.entered_up.get_value(t) <= model.ON_UP.get_value(t))
        model.add_constraint(
            model.entered_up.get_value(t)
            >= model.ON_UP.get_value(t) - model.ON_UP.get_value(t - model.parameters.date.timestep)
        )
        # entered_down (eq. (8))
        model.add_constraint(
            model.entered_down.get_value(t) <= 1 - model.ON_DOWN.get_value(t - model.parameters.date.timestep)
        )
        model.add_constraint(model.entered_down.get_value(t) <= model.ON_DOWN.get_value(t))
        model.add_constraint(
            model.entered_down.get_value(t)
            >= model.ON_DOWN.get_value(t) - model.ON_DOWN.get_value(t - model.parameters.date.timestep)
        )
    ...
    for t in model.gradients_time_frame:
        t_plus_one = t + model.parameters.date.timestep
