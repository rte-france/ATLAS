...
def execute(model: ThermalOptimization) -> None:
    ...
    for t in model.time_frame:
        ...
        # Enforces eq. (9)
        model.add_constraint(model.OFF[t] + model.ON_UP[t] + model.ON_DOWN[t] == 1)
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(
            model.get_variable(model.reserves_up_equip_at(t)) <= model.q_upper.get_value(t) * (1 - model.OFF[t])
        )
        model.add_constraint(
            model.get_variable(model.reserves_down_equip_at(t)) <= model.q_upper.get_value(t) * (1 - model.OFF[t])
        )

        # Power output
    for t in model.time_frame:
        model.add_constraint(
            model.q[t] >= model.q_lower.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t]),
            f"lower_bound_of_{model.thermal_unit.name}_at_{t}",
        )  # Lower bound (eq. 33)

        model.add_constraint(
            model.q[t] <= model.q_upper.get_value(t) * (model.ON_UP[t] + model.ON_DOWN[t]),
    ...
    if model.delta_q > 0:  # Case where the gradient is finite.
        for t in model.gradients_time_frame:  # The gradients are defined only up to T-1.
            ...
            model.add_constraint(
                ...
                <= model.delta_q * model.ON_UP[t] + model.delta_q_unconstrained * model.turned_on[t_next],
            ...
            model.add_constraint(
                model.q[t_next] - model.q[t]
                >= -model.delta_q * model.ON_DOWN[t] - model.delta_q_unconstrained * model.turned_off[t_next],
        ...
        for t in model.gradients_time_frame:
            ...
            model.add_constraint(
                model.q[t_next] - model.q[t]
                <= model.delta_q_unconstrained * model.ON_UP[t] + model.delta_q_unconstrained * model.turned_on[t_next]
            )  # Upward gradient
