...
def execute(model: ThermalOptimizationModel, day_zero: bool) -> None:
    ...
    for t in model.time_frame:
        ...
        # Enforces eq. (9)
        model.add_constraint(
            model.OFF.get_value(t) + model.ON_UP[t] + model.ON_DOWN.get_value(t) + model.STOP[t] + model.START[t] == 1,
    ...
    for t in model.time_frame:
        ...
        # ON to OFF (eq.(18) )
        model.add_constraint(model.ON_UP[t_minus_one] + model.OFF.get_value(t) <= 1)
    ...
    for t in model.time_frame:
        ...
        model.add_constraint(
            ...
            <= model.q_upper.get_value(t) * (1 - model.OFF.get_value(t) - model.START[t] - model.STOP[t])
        )
        model.add_constraint(
