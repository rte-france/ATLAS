...
class MarketClearingOutputDataset(AbstractDataset[MarketClearingParameters]):
    ...
    def update_raw_data_with_not_modified_business_model_object(self):
        ...
        for business_model in cfg.INVERSE_MODEL_MAPPING_NAME:
            if business_model not in self.get_custom_business_model_object_modified():
                business_model_name = cfg.INVERSE_MODEL_MAPPING_NAME[business_model]
    ...
    def update_raw_data_with_modified_business_model_object(self):
        ...
        for mc_market_area in self.input_dataset.mc_market_areas.items():
    ...
    def update_orders(self, accepted_powers: dict[tuple[str, str], float], market_prices: dict[tuple[str, int], float]):
        null_timeseries = Timeseries.from_index(
            self.input_dataset.parameters.times[0],
            self.input_dataset.parameters.time_step,
            self.input_dataset.parameters.times[-1],
            0.0
        )
        # Updating orders
            ...
            if not mc_order.is_agent_tso:
                ...
                match self.input_dataset.parameters.market:
                    ...
                    case Product.Intraday:
                        equipment.total_id_cleared_quantity = self.update_timeseries(
                            equipment.total_id_cleared_quantity, value_sold_ts)
                        portfolio.total_id_cleared_quantity = self.update_timeseries(
    ...
    def update_market_area(self, local_balances: dict[tuple[str, int], float],
        ...
        for market_area_name, mc_market_area in self.input_dataset.mc_market_areas.items():
            balance_values = [local_balances[market_area_name, time_index] for time_index in len(self.input_dataset.times)]
            price_values = [market_prices[market_area_name, time_index] for time_index in len(self.input_dataset.times)]
            ...
            match self.input_dataset.parameters.market:
                case Product.DayAhead:
                    ...
                    mc_market_area.da_balance = self.update_timeseries(mc_market_area.da_balance, values_bal)
                    mc_market_area.da_price += values_price
                case Product.Intraday:
                    mc_market_area.total_id_balance = self.update_timeseries(mc_market_area.total_id_balance, values_bal)
    ...
    def update_market_border(self, border_exchanges: dict[tuple[str, int], float],
        ...
        for market_border_name, mc_market_border in self.input_dataset.mc_market_borders.items():
            ...
            flow_values = [border_exchanges[market_border_name, time_index]
                           for time_index in len(self.input_dataset.times)]
            shadow_price_values = [
                market_prices[mc_market_border.uphill_market_area, time_index] -
                market_prices[mc_market_border.downhill_market_area, time_index]
                for time_index in len(self.input_dataset.times)]
            ...
            match self.input_dataset.parameters.market:
                ...
                case Product.Intraday:
                    mc_market_border.total_id_flow = self.update_timeseries(
        ...
        for critical_branch_name, mc_critical_branch in self.input_dataset.mc_critical_branches.items():
            flow_values = [
                sum([mc_market_area_ptdf.da_ptdf.get_value(time)
                     for mc_market_area_ptdf in mc_critical_branch.market_area_ptdf.values()])
                for time_index, time in enumerate(self.input_dataset.times)]
            ...
            match self.input_dataset.parameters.market:
                ...
                case Product.Intraday:
                    mc_critical_branch.total_id_flow = self.update_timeseries(
    ...
    def update_timeseries(self, ts_obj, ts_to_add):
        """
    ...
    def get_custom_business_model_object_modified(self) -> list[type[BusinessModel]]:
