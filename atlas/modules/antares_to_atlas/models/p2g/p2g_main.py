...
def P2G(antares_dataset, atlas_dataset, p):
    for antares_node in antares_dataset.Node.GetAllInstances():
        if antares_node.Name in p.market_areas_list:
            link_P2G_base = antares_dataset.Link.GetInstanceByName(f"{antares_node.Name}_z_p2g_base")
            link_P2G_marg = antares_dataset.Link.GetInstanceByName(f"{antares_node.Name}_z_p2g_marg")
            link_P2G_methanation = antares_dataset.Link.GetInstanceByName(f"{antares_node.Name}_z_p2g_methanation")

            # Base
            if link_P2G_base and link_P2G_base.DirectTransferCapacity.GetTimeSeriesByName("1").Abs().Max() != 0:
                msg = f"Creating a new base P2G load equipment in market area {antares_node.Name}"
                API.IO.Trace.Log(msg, API.IO.LogTypeInfo)

                p2g_instance = atlas_dataset.Equipment.Load.CreateInstance(f"{antares_node.Name}_p2g_base")

                # general properties
                if p.consumption_production_separation:
                    portfolio = atlas_dataset.MarketAgent.Portfolio.GetInstanceByName(f"supplier_{antares_node.Name}")
                else:
                    portfolio = atlas_dataset.MarketAgent.Portfolio.GetInstanceByName(f"portfolio_{antares_node.Name}")

                p2g_instance.Node = atlas_dataset.Network.Node.GetInstanceByName(antares_node.Name)
                ...
                # variable_cost = antares_dataset.ThermalTechnology.GetInstanceByName("z_P2G_base").MarketBidCost
            ...
            if link_P2G_marg and link_P2G_marg.DirectTransferCapacity.GetTimeSeriesByName("1").Abs().Max() != 0:
                msg = f"Creating a new marginal P2G load equipment in market area {antares_node.Name}"
                API.IO.Trace.Log(msg, API.IO.LogTypeInfo)

                p2g_instance = atlas_dataset.Equipment.Load.CreateInstance(f"{antares_node.Name}_p2g_marg")
                ...
                # VariableCost
                variable_cost = antares_dataset.ThermalTechnology.GetInstanceByName(
            ...
            if (
                ...
                msg = f"Creating a new methanation P2G load equipment in market area {antares_node.Name}"
                API.IO.Trace.Log(msg, API.IO.LogTypeInfo)

                p2g_instance = atlas_dataset.Equipment.Load.CreateInstance(f"{antares_node.Name}_p2g_methanation")
            ...
            thermic_h2 = atlas_dataset.Equipment.Thermic.CreateInstance(antares_node.Name+"_"+functions.node_special_format(antares_node.Name)+"_v_me_h2")
            thermic_ch4 = atlas_dataset.Equipment.Thermic.CreateInstance(antares_node.Name+"_"+functions.node_special_format(antares_node.Name)+"_v_me_ch4")

            #general properties
            if p.consumption_production_separation:
                portfolio = atlas_dataset.MarketAgent.Portfolio.GetInstanceByName("generator_"+antares_node.Name)
            else:
                portfolio = atlas_dataset.MarketAgent.Portfolio.GetInstanceByName("portfolio_"+antares_node.Name)

            thermic_h2.Node = atlas_dataset.Network.Node.GetInstanceByName(antares_node.Name)
            thermic_h2.Portfolio = portfolio

            thermic_ch4.Node = atlas_dataset.Network.Node.GetInstanceByName(antares_node.Name)
            ...
            v_me_h2 = antares_dataset.Node.GetInstanceByName("v_me_h2")
            v_me_ch4 = antares_dataset.Node.GetInstanceByName("v_me_ch4")

            #yields (<1)
            yield_h2 = yield_electrolysers_node(antares_dataset, antares_node.Name)
            yield_ch4 = yield_methanation_node(antares_dataset, antares_node.Name)
...
def yield_electrolysers_node(antares_dataset, node):
    # yield of thermal technology found in binding constraint
    bc_h2 = antares_dataset.BindingConstraint.GetInstanceByName("me_p2g_base")
    if not bc_h2:
...
def yield_methanation_node(antares_dataset, node):
    # yield of thermal technology found in binding constraint
    bc_ch4 = antares_dataset.BindingConstraint.GetInstanceByName("me_p2g_methanation")
    if not bc_ch4:
        API.IO.Trace.Log(
    ...
    link = f"{node}_z_p2g_methanation"
