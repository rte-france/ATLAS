...
class TestOutputShape:
    def test_one_local_balance_per_market_area_and_time(
        ...
        assert set(output_dataset.local_balances) == expected_keys
    ...
    def test_one_market_price_per_market_area_and_time(
        ...
        assert set(output_dataset.market_prices) == expected_keys
    ...
    def test_one_border_exchange_per_border_and_time(
        ...
        assert set(output_dataset.border_exchanges) == expected_keys
    ...
    def test_accepted_powers_reference_known_orders(
        ...
        for area_name, order_name in output_dataset.accepted_powers:
...
class TestOutputValues:
    def test_all_output_values_are_finite(self, output_dataset: MarketClearingOutputDataset) -> None:
        for value in output_dataset.local_balances.values():
            assert _is_finite(value)
        for value in output_dataset.market_prices.values():
            assert _is_finite(value)
        for value in output_dataset.border_exchanges.values():
            assert _is_finite(value)
        for value in output_dataset.accepted_powers.values():
            assert _is_finite(value)
    ...
    def test_border_exchanges_respect_capacity_bounds(
        ...
        for (border_name, time_index), exchange in output_dataset.border_exchanges.items():
    ...
    def test_market_prices_within_area_price_bounds(
        ...
        tolerance = input_dataset.parameters.allowed_round_off_error
        for (area_name, time_index), price in output_dataset.market_prices.items():
...
class TestChangeSets:
    def test_run_produces_non_empty_change_sets(self, output_dataset: MarketClearingOutputDataset) -> None:
        assert isinstance(output_dataset.change_sets, list)
        assert len(output_dataset.change_sets) > 0
