...
from atlas.core.orchestrator.actionplan.parameters import DataQualityWarning, Task
...
class TestTask:
    ...
    def test_custom_name_is_preserved(self, tmp_path, params_file):
        task = TestTask.build_task(
            name="task_name_test", module="PortfolioOptimisation", module_parameters_path=params_file
        )
    ...
    def test_both_module_and_workflow_raise(self, params_file, empty_workflow):
        with pytest.raises(Exception):
            TestTask.build_task(
                module="PortfolioOptimisation", workflow=empty_workflow, module_parameters_path=params_file
            )
    ...
    def test_parameters_path_is_path_object_module(self, tmp_path, params_file):
        ...
        assert isinstance(task.module_parameters_path, Path)
    ...
    def test_datetime_are_preserved(self, params_file, empty_workflow):
        ...
        task = TestTask.build_task(
            from_=from_, until=until, module_parameters_path=params_file, module="PortfolioOptimisation"
        )
