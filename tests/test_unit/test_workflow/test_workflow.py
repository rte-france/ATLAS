...
def test_build_step_unknown_module_raises(workflow_parameters, step_file):
    wf = Workflow(workflow_parameters)
    with pytest.raises(ValueError):
        wf.build_step(workflow_parameters.steps[0])
...
def test_get_output_dataset_returns_last_step_output(mock_get, workflow_parameters):
    ...
    wf.steps[0]._output_dataset = last_dataset
