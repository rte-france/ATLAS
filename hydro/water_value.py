...
def node_water_value_computation(antares_node, hydro, inflows_dictionary, p):
    ...
    if scenario_parameter == "All":
        ...
        for s in range(len(scenarios)):
            ...
            if not scenarios[s] in available_scenarios:
                msg = "Cannot compute water values for {}: the scenario \'{}\' specified in the parameter WaterValueScenarios does not exist " \
                    "in the CalculatedMarginalPrice matrix of node {}. Available price scenarios in {} are: {}".format(hydro.Name, scenarios[s], antares_node.Name, antares_node.Name, available_scenarios)
                raise Exception(msg)
    ...
    one_year_hours_index = API.DatetimeIndex.NewIndex(p.start_date, p.start_date.AddYears(1), str(p.water_value_time_step)+'m')
                        ...
                        for j in range(p.hydro_storage_subdivision+1):
                            ...
                            if j == 0:
                                ...
                                if n != level and n < len(Stock):
                                    G2 = BV[t+1][n] + (Stock[level] + Inflows_t - Stock[n])/(Stock[n]-Stock[n-1])*(BV[t+1][n]-BV[t+1][n-1]) 
                                G = max(G1,G2)
                                ...
                                if Stock_j > 0:
                                    ...
                                    if level == 1: 
                                        if p.use_bellman_interpolation:
                                            ...
                                            stock_value = p.beta *  ((BV[t+1][m] -BV[t+1][m+1])*(Stock[m+1]-Stock_j)/(Stock[m+1]-Stock[m])+BV[t+1][m+1])
                                        else:
    ...
    if p.nb_storage_levels == 0 or M == 0:
        for level in range(1,len(Stock)) :
            my_TimeSeries = API.TimeSeries.NewTimeSeries(str(level), API.TimeSeries.Linear, "MW", one_year_hours_index, 0) 
            for t in range(len(one_year_hours_index)):
        ...
        levels_cutoff = levels[int(delta/2) if delta % 2 == 0 else delta - int(delta/2) :len(levels)- int(delta/2)]
